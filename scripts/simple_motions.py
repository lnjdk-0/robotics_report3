...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if detect_ball and t_pose is not None:
            ...
            try:
                trans = buffer.lookup_transform('base',
                        'camera_color_optical_frame', rospy.Time())
            ...
            print (
                'Test point in the CAM frame:  x= ',
                format(cam.point.x, '.3f'),
                '(m), y= ',
                format(cam.point.y, '.3f'),
                '(m), z= ',
                format(cam.point.z, '.3f'),
                '(m)',
                )
            print (
                'Transformed point in the BASE frame:  x= ',
                format(base.point.x, '.3f'),
                '(m), y= ',
                format(base.point.y, '.3f'),
                '(m), z= ',
                format(base.point.z, '.3f'),
                '(m)',
                )
            print ('-------------------------------------------------')
            ...
            (rx, ry, rz) = (t_pose.linear.x, t_pose.linear.y,
                            t_pose.linear.z)
            (roll, pitch, yaw) = (t_pose.angular.x, t_pose.angular.y,
                                  t_pose.angular.z)
            ...
            plan.modes.append(mode1)  # modified Plan.msg inside ur5e_control package and added "modes" array

            # Above the ball

            (point2, mode2) = add_point(
                base.point.x,
                base.point.y,
                base.point.z + ball.radius + 0.1,
                roll,
                pitch,
                yaw,
                0,
            ...
            (point3, mode3) = add_point(
                base.point.x,
                base.point.y,
                base.point.z + ball.radius,
                roll,
                pitch,
                yaw,
                2,
